...
function go_detail(pageNum, numPerPage) {
	var theForm = document.frm;
	// 상품 상세 보기 페이지에서 다시 상품 리스트로 돌아왔을 경우 현재 페이지로
	// 돌아올 수 있도록 하기 위해서 현재 페이지 번호를 쿼리 스트링으로 넘겨준다.
	theForm.action =  "admin_product_detail?pageNum=" + pageNum + "&numPerPage=" + numPerPage;
...
// **************** productDetail.jsp
function go_mod(pageNum, numPerPage) {
	var theForm = document.frm;
	//현재 페이지를 쿼리 스트링으로 넘긴다.
	theForm.action = "admin_product_update_form?pageNum=" + pageNum + "&numPerPage=" + numPerPage;
	theForm.submit();
}

function go_mod_save(pageNum, numPerPage) {
	var theForm = document.frm;
	...
	if (theForm.kind.value == '') {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			theForm.action = "admin_product_update?pageNum=" + pageNum + "&numPerPage=" + numPerPage;
			theForm.submit();
		}
	}
}

function go_mod_mov(pageNum, numPerPage) {
	var theForm = document.frm;
	theForm.action = "command=admin_product_detail?pageNum=" + pageNum + "&numPerPage=" + numPerPage;
	theForm.submit();
}
